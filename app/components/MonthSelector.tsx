...
export function MonthSelector({ selectedMonth, onMonthChange, selectedYear, onYearChange }: MonthSelectorProps) {
  const [isMoreOpen, setIsMoreOpen] = useState(false);
  const [isYearOpen, setIsYearOpen] = useState(false);

  const recentMonths = ["may", "june", "july"];
  ...
  // Dynamically compute which pills to show.
  // If the selected month is a historical month (e.g. March), it replaces the oldest pill (May)
  // so the selected month is always visible as a pill.
  const pills = recentMonths.includes(selectedMonth)
    ? recentMonths
    : [selectedMonth, "june", "july"];
  ...
  return (
    <div className="flex items-start gap-2 relative">
      ...
      <div className="relative">
        <Pill
          active={!recentMonths.includes(selectedMonth)}
          onClick={() => setIsMoreOpen(!isMoreOpen)}
          className="px-3 gap-0.5"
        >
          More
          <span className="material-symbols-outlined text-[16px] leading-none font-bold">keyboard_arrow_down</span>
        </Pill>
        ...
        {isMoreOpen && (
          <>
            ...
            <div className="absolute right-0 mt-2 w-44 bg-surface-container-lowest/90 dark:bg-inverse-surface/90 backdrop-blur-xl border border-surface-variant/50 rounded-xl shadow-lg z-50 py-1.5 px-1 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
              <span className="block px-3 py-1 text-[9px] uppercase tracking-wider text-on-surface-variant/60 font-bold mb-1">Select Month</span>
              {allMonths.map((month) => (
                <button
                  key={month.id}
                  onClick={() => {
        ...
        {isYearOpen && (
          <>
            ...
            <div className="absolute right-0 mt-2 w-32 bg-surface-container-lowest/90 dark:bg-inverse-surface/90 backdrop-blur-xl border border-surface-variant/50 rounded-xl shadow-lg z-50 py-1.5 px-1 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
              <span className="block px-3 py-1 text-[9px] uppercase tracking-wider text-on-surface-variant/60 font-bold mb-1">Select Year</span>
              {years.map((year) => (
                <button
                  key={year}
                  onClick={() => {
